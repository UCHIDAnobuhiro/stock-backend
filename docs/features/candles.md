...
- **複数銘柄の一括取得（Quotes）**: 最新終値・前日比・スパークライン用終値配列を複数銘柄まとめて取得（ウォッチリストのN+1回避）
...
| パラメータ | デフォルト | 説明 |
|-----------|-----------|------|
| `codes` | なし（必須） | カンマ区切りの銘柄コード（1〜50件）。各コードは `code` パスパラメータと同じパターンに一致する必要がある。重複コードは順序を保って除去される |
| `interval` | `1day` | 時間間隔（`1day`, `1week`, `1month`） |
| `bars` | `0` | スパークライン用に含める直近終値の本数（`0`〜`500`）。`0`の場合はレスポンスに`closes`を含めない |
