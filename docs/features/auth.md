...
sequenceDiagram
    ...
    alt OAuthAccount Found
        ...
        alt User Found (同メールで登録済)
            ...
            Handler-->>Client: 302 Redirect → {FRONTEND_URL}/login?error=account_conflict
        else User Not Found
...
- **409 Conflict**: 同じトークンによる更新が並行中（Cookieは維持、`Retry-After: 1`）
- **429 Too Many Requests**: IP単位で30回/分を超過

409を受け取った場合、先行リクエストのレスポンスによってブラウザの`refresh_token` Cookieが
更新される可能性があるため、`Retry-After`に従って1秒程度待機した後、**1回だけ**再試行します。
再試行を繰り返すと429に達し、30秒の再利用猶予を超えた後は401とセッション系列失効になるため、
フロントエンド側では更新要求を単一化してください。
