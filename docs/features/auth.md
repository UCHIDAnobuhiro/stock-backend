...
- **OAuth2 ログイン**: Google / GitHub プロバイダーによるソーシャルログイン（PKCE 対応。同メールの既存アカウントへの自動リンクは行わず 409 を返す）
...
sequenceDiagram
    ...
    alt OAuthAccount Found
        ...
        alt User Found (同メールで登録済)
            ...
            Usecase-->>Handler: ErrOAuthEmailConflict<br/>(乗っ取り防止のため自動リンクは行わない)
...
**Cookie**
- `oauth_state`: 認可開始時に発行した state を保持する短命 Cookie（必須）。クエリの `state` と一致しない場合は 400。ログイン CSRF / セッションフィクセーション対策。
...
- **400 Bad Request** - state が不正・期限切れ、`oauth_state` Cookie の欠落・不一致、または code/state 欠落、未対応のプロバイダー
  ```json
  { "error": "invalid or expired state" }
  ```
- **409 Conflict** - 同メールアドレスの既存アカウントが存在（乗っ取り防止のため自動リンクは行わない）
  ```json
...
graph TB
    ...
    subgraph "Usecase Layer"
        Usecase[Usecase<br/>auth]
    end

    subgraph "Domain Layer"
        Entity[User Entity<br/>auth]
    end
...
- OAuth アカウントの明示的リンク承認フロー（既存パスワードでの再認証や確認メールによる本人確認を経て、ログイン済みユーザーに OAuth アカウントを紐付ける）
